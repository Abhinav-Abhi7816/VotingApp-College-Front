...
function Candidate({candidate}) {
  ...
  async function voteClick(SlNo,nm)
  {
    ...
    try{
      const response=await fetch('https://votingapp-college-back.onrender.com/submitVote',{
        method:'POST',
        headers:{
    ...
    <div>
      <div className='flex md:text-2xl text-xs w-fit md:px-10 md:gap-10 rounded-xl' style={{boxShadow:"0px 0px 8px rgb(0,0,0,0.3"}}>
        <div className='flex place-items-center'>
            ...
            <img src={candidate.imgUrl} alt={candidate.name} className='md:size-48 sm:size-20 size-12 rounded-full shadow-xl'></img>
            </div>
        </div>
        <div className='md:text-8xl text-lg flex place-items-center border-l-4 md:min-w-28 sm:min-w-12 min-w-10 justify-center'>
            <p>{candidate.SlNo}</p>
        </div>
        <div className='flex flex-col w-full justify-center md:gap-5 gap-1 min-w-32 md:min-w-96 border-r-4 py-2 md:py-0 max-w-64 md:max-w-96'>
            <p className='md:text-3xl text-wrap flex flex-nowrap gap-2'><span className='md:block hidden'>Name:</span> {candidate.name}</p>
            <div className='flex flex-col md:flex-row md:gap-5 gap-1 '>
            <p className='flex flex-nowrap'><span className='hidden'>Post :</span> {candidate.post}</p>
            <p className='flex'><span className='hidden md:flex'>Dept :</span> {candidate.dept}</p>
            </div>
        </div>
