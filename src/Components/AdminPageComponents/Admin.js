...
function Admin() {
  ...
  async function getCanData() {
    try {
      let response = await fetch('https://votingapp-college-back.onrender.com/candidates/allData', {
        method: "GET",
  ...
  async function getVotersDetails() {
    try {
      let response = await fetch('https://votingapp-college-back.onrender.com/votersList', {
        method: "GET",
        headers: { "Authorization": `Bearer ${JSON.parse(localStorage.getItem('token'))}` }
      });
      let result = await response.json();
  ...
  useEffect(() => {
    async function getVotesCount() {
      try {
        let response = await fetch('https://votingapp-college-back.onrender.com/votesCount', {
          method: "GET",
          headers: { "Authorization": `Bearer ${JSON.parse(localStorage.getItem('token'))}` }
        });
        let result = await response.json();
  ...
  return (
    <div className='lg:pt-32 pt-20 min-h-lvh w-full'>
      <div className='flex flex-col place-items-center mx-2 my-3 md:mx-8 md:my-4  rounded-xl' style={{ boxShadow: "0px 0px 20px rgb(0,0,0,0.3)" }}>
        <div className='py-4'><h1 className='md:text-3xl text-xl font-semibold border-b-4 px-4 border-black'>Vote Count : Event coordinator </h1></div>
